...
def solver(b, q, I, V, f, Lx, Ly, Nx, Ny, dt, T,user_action=None, version='scalar',plug='no',exact='no'):
		...
		x_g = linspace(0-dx, Lx+dx, Nx+3) # mesh points in x dir
		y_g = linspace(0-dy, Ly+dy, Ny+3) # mesh points in y dir
		...
		if version == 'scalar':
			...
			u = advance(x, x_g, y, y_g, b,q,u, u_1, u_2,u_g, u_1_g, u_2_g, f_a, Cx2, Cy2, dt, V, step1=True)
...
def advance_vectorized(x, x_g, y, y_g, b, q, u, u_1, u_2, u_g, u_1_g, u_2_g, f_a, Cx2, Cy2, dt,V, step1=False):
		...
		#Add ghostcells on north
		u_g[1:-1,-1] = u[:,-2]
		u_1_g[1:-1,-1] = u_1[:,-2]
		u_2_g[1:-1,-1] = u_2[:,-2]
		
		#Add ghostcells on west
		u_g[0,1:-1] = u[1,:]
		u_1_g[0,1:-1] = u_1[1,:]
		u_2_g[0,1:-1] = u_2[1,:]
		
		#Add ghostcells on east
		u_g[-1,1:-1] = u[-2,:]
		u_1_g[-1,1:-1] = u_1[-2,:]
		u_2_g[-1,1:-1] = u_2[-2,:]
		...
		"""
		u_xqu_x  = Cx2*( 0.5*( (q(x_g[2:],y_g[1:-1])+q(x_g[1:-1],y_g[1:-1]) )*(u_1_g[2:,1:-1]-u_1_g[1:-1,1:-1])) \
				-0.5*( (q(x_g[:-2],y_g[1:-1])+q(x_g[1:-1],y_g[1:-1]) )*(u_1_g[1:-1,1:-1]-u_1_g[:-2,1:-1]) ))


		u_yqu_y  = Cy2*( 0.5*( (q(x_g[1:-1],y_g[2:])+ q(x_g[1:-1],y_g[1:-1]) ) * (u_1_g[1:-1,2:]-u_1_g[1:-1,1:-1]) ) \
				-0.5*( (q(x_g[1:-1],y_g[:-2])+ q(x_g[1:-1],y_g[1:-1]) ) * (u_1_g[1:-1,1:-1]-u_1_g[1:-1,:-2]) ))
		

		u[:,:] = (1/A)*( (2/R1)*u_1_g[1:-1,1:-1] +B*(R2/R1)*u_2_g[1:-1,1:-1] - C*(R2/R1)*dt*V(x_g[1:-1],y_g[1:-1])  + (1/R1)*(u_xqu_x +\
				u_yqu_y + dt2*f_a[:,:]) )
